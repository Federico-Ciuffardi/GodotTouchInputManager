...
func updateScreenDrag(event : InputEventScreenDrag, time : float = -1):
...
func updateScreenTouch(event : InputEventScreenTouch, time : float = -1):
	time     = _now() if time < 0 else time
	var touch = Touch.new()
...
func isConsistent(diff_limit : float, length_limit : float = -1):
	if length_limit == -1: length_limit = length_limit
	var valid = true
	var i = 0
	var presses_centroid  = centroid("presses", "position")
	var releases_centroid = centroid("releases", "position")
	while i < size() and valid:
		var press_relative_position   = presses[i].position   - presses_centroid
		var release_relative_position = releases[i].position - releases_centroid
